...
func freshStoreWithChat(c domain.Chat) store.Store {
	s := store.NewMemory()
	s.SetChat(c)
	return s
}
...
func TestNotifiable_FreshUnmutedOtherChat(t *testing.T) {
	s := freshStoreWithChat(domain.Chat{ID: 7, Title: "Alice"})
	now := time.Now()
...
func TestNotifiable_CurrentChatSuppressed(t *testing.T) {
	s := freshStoreWithChat(domain.Chat{ID: 7})
	now := time.Now()
...
func TestNotifiable_MutedSuppressed(t *testing.T) {
	s := freshStoreWithChat(domain.Chat{ID: 7, IsMuted: true})
	now := time.Now()
	if store.Notifiable(s, 7, 0, now, now) {
...
func TestNotifiable_ArchivedSuppressed(t *testing.T) {
	s := freshStoreWithChat(domain.Chat{ID: 7, IsArchived: true})
	now := time.Now()
	if store.Notifiable(s, 7, 0, now, now) {
